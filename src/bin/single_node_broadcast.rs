...
impl Node for Handler {
    async fn process(&self, runtime: Runtime, req: Message) -> Result<()> {
        if req.get_type() == "broadcast" {
            ...
            let message = req.body.extra.get("message").expect("Expected a message value from broadcast");

            if let Value::Number(value) = message {
                let mut store = self.store.write().await;
                store.push(value.as_i64().expect("Expected a valid number in the message"));
            ...
            let json_messages: Vec<Value> = store.iter().map(|x| Value::Number(Number::from(*x))).collect();

            response.extra.insert("messages".to_string(), Value::Array(json_messages) );
            return runtime.reply(req, response).await;
